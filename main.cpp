...
#define INC_SPEED 0.05f  //menambah kecepatan
...
void ZCylinder(GLfloat radius,GLfloat length)
{
    ...
    glPushMatrix();
    glTranslatef(0.0f,0.0f,0.0f); //bongkar motor
...
void drawFrame()
{
    ...
    glPushMatrix();
    glTranslatef(0.0f,0.0f,0.06f); //hilang gir antara yang atas dengan bawah nya
    glRotatef(-2*pedalAngle,0.0f,0.0f,1.0f);
    gear(0.08f,0.3f,0.03f,30,0.03f);
    glPopMatrix();

    glColor3f(1.0f,0.0f,1.0f); //kerangka badan V
    ...
    glColor3f(1.0f,0.0f,1.0f); //ubah warna antara gir depan dan belakang
    glPopMatrix();
    ...
    glPushMatrix();
    glRotatef(-2*pedalAngle,0.0f,0.0f,1.0f);
    drawTyre(); //hilangkan ban belakang
    glColor3f(0.0f,1.0f,0.0f);
    gear(0.03f,0.15f,0.03f,20,0.03f);
    glColor3f(1.0f,1.0f,1.0f); //kerangka belakang
    glPopMatrix();
    glRotatef(LEFT_ANGLE,0.0f,0.0f,1.0f); //rangka depan
    ...
    glTranslatef(WHEEL_LEN,0.0f,0.0f); //angkat ban
    ...
    glPushMatrix();
    glColor3f(1.0f,0.0f,1.0f); //tabung atas
    glTranslatef(-0.6,-0.6f,0.0f);
    XCylinder(GAS_TANK,1.0f);
    glPopMatrix();

    glPushMatrix();
    glColor3f(1.0f,0.0f,1.0f); //tabung senter
    ...
    ZCylinder(0.07f,HANDLE_ROD/4);
    glColor3f(1.0f,0.0f,1.0f); //warna rangka keseluruhan
    glPopMatrix();
    glPopMatrix();
...
void drawChain()
{
    GLfloat depth;
    static int mode=0;
    glColor3f(1.0f,0.0f,1.0f); //ubah warna rantainya
...
void drawTyre(void)
{
    ...
    glColor3f(1.0f,0.0f,1.0f); //ubah warna ban dalam kecil
    ...
    {
        glPushMatrix();
        glRotatef(i*SPOKE_ANGLE,0.0f,0.0f,1.0f); //menambah ban belakang
        glBegin(GL_LINES);
    ...
    glColor3f(1.0f,0.0f,1.0f); //ubah warna ban
...
{
    // printf("%s\n", "YESS ");
    GLfloat light_directional[]={1.0,1.0,1.0,1.0}; //hilang motor
...
{
  reset();
	glClearColor  (0.50,0.10,0.9,0);
}

void draw_ground()
{
	glClearColor  (0.50,0.10,0.9,0); //ubah warna background
	//glEnable(GL_TEXTURE_2D);
...
void display_bike(void)
{
    ...
    glEnable(GL_NORMALIZE); //HILANGKAN MOTOR
